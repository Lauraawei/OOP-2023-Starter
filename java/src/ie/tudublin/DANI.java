	...
	public void loadFile() {
		...
		for (String line : lines) {
		  ...
		  for (int i = 0; i < words.length - 1; i++) {
			...
			if (word == null) {
			  model.add(new Word(words[i], new ArrayList<>()));
			  word = findWord(words[i]);
			}
			Follow follow = null;
			...
			if (follow == null) {
			  word.getFollows().add(new Follow(words[i+1], 1));
			} else {
			  follow.count++;
			}
		  }
		  if (words.length > 0) {
			Word word = findWord(words[words.length - 1]);
			if (word == null) {
			  model.add(new Word(words[words.length - 1], new ArrayList<>()));
			}
		  }
		}
	  }
